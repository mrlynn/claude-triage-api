 ...
 * audio:    the page has narration. When static/audio/<out>.mp3 exists
 *           (generated by scripts/generate-audio.mjs and committed), the page
 *           gets `audio_src` frontmatter and DocItem/Content renders a
 *           player, and a lab listed in src/data/lab-videos.json also gets
 *           `video_id` and the embedded video.
 *           No MP3 yet means no player, so audio generation can lag
 *           content changes without breaking anything.
 */
/**
 * Lab slug to YouTube id, hand-maintained.
 *
 * A page listed here gets `video_id` frontmatter and DocItem/Content renders
 * the video above the lab; a page missing from it renders nothing, the same
 * way a missing MP3 means no audio player. Uploading is a manual step, so this
 * is the one piece of the video pipeline a human has to write down — the ids
 * cannot be derived from anything in the repo.
 */
const LAB_VIDEOS = JSON.parse(
  readFileSync(join(websiteDir, "src", "data", "lab-videos.json"), "utf8"),
);
...
const PAGES = [
  ...
  { source: "curriculum/guides/claude-api-tutorial.md", out: "guides/claude-api-tutorial", position: 1, title: "Claude API tutorial" },
  { source: "curriculum/guides/structured-outputs-typescript-zod.md", out: "guides/structured-outputs-typescript-zod", position: 2, title: "Structured outputs with TypeScript and Zod" },
  { source: "curriculum/guides/claude-tool-use.md", out: "guides/claude-tool-use", position: 3, title: "Claude tool use" },
  { source: "curriculum/guides/prompt-caching-and-evals.md", out: "guides/prompt-caching-and-evals", position: 4, title: "Prompt caching and evals" },
  { source: "curriculum/labs/lab-0-scoreboard.md", out: "labs/lab-0-scoreboard", position: 0, audio: true },
  { source: "curriculum/labs/lab-1-first-call.md", out: "labs/lab-1-first-call", position: 1, audio: true },
  { source: "curriculum/labs/lab-2-structured-outputs.md", out: "labs/lab-2-structured-outputs", position: 2, audio: true },
  { source: "curriculum/labs/lab-3-tool-use.md", out: "labs/lab-3-tool-use", position: 3, audio: true },
  { source: "curriculum/labs/lab-4-streaming.md", out: "labs/lab-4-streaming", position: 4, audio: true },
  { source: "curriculum/labs/lab-5-prompt-caching.md", out: "labs/lab-5-prompt-caching", position: 5, audio: true },
  { source: "curriculum/labs/lab-6-evals.md", out: "labs/lab-6-evals", position: 6, audio: true },
  { source: "curriculum/labs/lab-7-choosing-a-model.md", out: "labs/lab-7-choosing-a-model", position: 7, audio: true },
  { source: "curriculum/labs/lab-8-trust-boundary.md", out: "labs/lab-8-trust-boundary", position: 8, audio: true },
  { source: "curriculum/labs/lab-9-shipping-it.md", out: "labs/lab-9-shipping-it", position: 9, audio: true },
  { source: "curriculum/labs/lab-10-ask-northwind.md", out: "labs/lab-10-ask-northwind", position: 10, audio: true },
...
function transform(page) {
  ...
  const frontmatter = [
    ...
    ...(page.audio &&
    existsSync(join(websiteDir, "static", "audio", `${page.out}.mp3`))
